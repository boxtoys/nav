(function() {
  ...
  function render(data) {
    ...
    const html = tpl.replace('{name}', data.name).replace('{icon}', data.icon).replace('{desc}', data.desc).replace('{link}', data.link.replace(/(https?:\/\/)/g, '')).replaceAll('{url}', data.link).replace('{round}', data.round === 1 ? 'round' : '')
