(function() {
  ...
  function render(data) {
    ...
    const html = tpl.replace('{name}', data.name).replace('{icon}', data.icon).replace('{desc}', data.desc).replace('{link}', data.link.replace(/(https?:\/\/)/g, '')).replace('{url}', data.link).replace('{round}', curr.round === 1 ? 'round' : '')
